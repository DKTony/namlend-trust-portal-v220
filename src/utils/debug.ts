...
const safeStringify = (obj: unknown, maxDepth: number = 3): string => {
  ...
  const replacer = (_key: string, value: unknown, depth: number = 0): unknown => {
    if (depth > maxDepth) {
    ...
    if (Array.isArray(value)) {
      return value.slice(0, 10).map((item, index) => replacer(`[${index}]`, item, depth + 1));
    }

    const safeObj: Record<string, unknown> = {};
    const record = value as Record<string, unknown>;
    const keys = Object.keys(record).slice(0, 20); // Limit object keys

    for (const k of keys) {
      try {
        safeObj[k] = replacer(k, record[k], depth + 1);
      } catch (err) {
