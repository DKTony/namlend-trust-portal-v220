...
## [4.2.0] - 2026-08-11 (Google Sign-In, Enrollment, KYC Document Workflow)

### Added

- **Google OAuth sign-in** (`convex/auth.ts`, `src/components/auth/GoogleSignInButton.tsx`)
  behind a server-side flag: the button renders only when the deployment reports
  `authProviders.listEnabled → { google: true }` (both `AUTH_GOOGLE_ID` and
  `AUTH_GOOGLE_SECRET` set). Cross-provider account linking is verified-email-only:
  a Google sign-in onto an unverified password account is blocked with guidance to sign
  in with the password (pre-hijack defence), and password flows never email-link at all
  (prevents grafting a password onto an existing Google identity).
- **Idempotent enrollment** (`convex/lib/enrollment.ts`): one routine guarantees the
  `profiles` + `userRoles` rows for every sign-in path, self-heals stranded sessions,
  fills blanks without clobbering, never downgrades roles, and audits first enrollment.
  `users.completeEnrollment` replaces the never-called `ensureProfile`; Google sign-ups
  complete phone + ID number once via `ProfileCompletionGate`.
- **Versioned KYC/loan document workflow** (`convex/kycDocuments.ts`,
  `convex/loanDocuments.ts`, `src/pages/KYC.tsx`, `src/components/documents/`):
  supersede-in-place versioning (no deletes, 7-year retention), per-document staff
  review with a KYC approval pipeline (`KycReviewPanel`), server-authoritative file
  validation (`convex/lib/documentPolicy.ts`), and legacy-metadata backfill tooling
  with per-page audit entries.
- **Short-lived document access grants** (`convex/lib/documentGrants.ts`,
  `GET /documents/fetch`): preview/download URLs now expire after 5 minutes and every
  actual fetch is audited. Raw `storage.getUrl()` links — permanent and
  unauthenticated — are no longer surfaced for compliance documents.

### Fixed

- Password sign-up silently discarded `name`/`phone` (bare `Password` provider used
  `defaultProfile`) — every profile was created with `fullName: undefined`.
- `profiles.idNumber` had no server write path at all; it is now set once via
  `completeEnrollment` (audited) and deliberately absent from `updateMyProfile`.
- Admin KYC queue rendered empty on deployments with unstamped tenancy
  (`undefined === null` strict-equality filter in `approvalWorkflow`).
- **Sign-out no longer hands the next user the previous user's destination.** The guard
  redirects to `/auth?next=<current page>` on session loss, which is right for an expired
  session but wrong for a deliberate sign-out — and the parameter was consumed by whoever
  signed in next, so on a shared device an admin signing in after a client landed on the
  client's dashboard instead of `/admin`. `signOut` now marks the exit as deliberate
  (`markDeliberateSignOut` in `src/lib/routing.ts`) and the guard drops `?next=`.
- `submitMyKyc` now asserts phone + ID number server-side (the browser gate is UX only).
- OAuth `?next=` open redirect closed (`src/utils/safeRedirect.ts`, shared with
  `ProtectedRoute`).
