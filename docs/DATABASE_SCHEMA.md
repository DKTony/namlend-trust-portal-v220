...
| `vpaRegistry`               | `userId`, `vpa`, `status`, `bankAccountId` _(legacy — being replaced by ipsAliasDirectory)_                                                                                                                                                                                                                                                                 | `by_userId`, `by_vpa`                                                 |
| `ipsAliasDirectory`         | `userId`, `addr`, `entityType` (PERSON/ENTITY), `idType` (MOBILE/NUMERICID), `idValue`, `status` (NEW/ACTIVE/INACTIVE/BLOCKED/DEREGISTERED/PORTED), `cmId?`, `syncedWithIps`, `isDefault`, `linkedAccountRef?`, `linkedBankBic?`                                                                                                                            | `by_addr`, `by_userId`, `by_status`, `by_idValue`, `by_syncedWithIps` |
| `ipsApiLogs`                | `method`, `endpoint`, `requestBody`, `responseStatus?`, `responseBody?`, `durationMs?`, `direction` (OUTBOUND/INBOUND/CALLBACK), `contentType` (json/xml), `apiName?`, `rawXml?`, `correlationId?`, `transactionId?`                                                                                                                                        | `by_transactionId`                                                    |
| `ipsAlerts`                 | `transactionId?`, `alertType`, `severity` (info/warning/critical), `message`, `isResolved`, `resolvedAt?`, `resolvedBy?`                                                                                                                                                                                                                                    | `by_isResolved`                                                       |
| `ipsOnboardingApplications` | `userId`, `status` (14 IPS-mandated states + 8 legacy states), `deviceBindingId?`, `mobileNumberNormalized?`, `sovProviderCode?`, `sovProviderName?`, `selectedAccountRef?`, `selectedAccountMasked?`, `selectedAccountIfsc?`, `verificationMethod?` (debit_card/mno), `ipsPinSet?`, `aliasAddr?`, `aliasId?`, `lastErrorCode?`, `lastErrorMessage?`        | `by_userId`, `by_status`                                              |
| `ipsDeviceBindings`         | `userId`, `onboardingId`, `deviceId`, `deviceName?`, `publicKey`, `status` (active/revoked/expired), `boundAt`                                                                                                                                                                                                                                              | `by_userId`                                                           |

**IPS Onboarding Status Values** (IPS-mandated): `NOT_STARTED`, `DEVICE_BINDING_REQUIRED`, `DEVICE_BOUND`, `SOV_SELECTION_PENDING`, `SOV_SELECTED`, `ACCOUNTS_LISTED`, `VERIFICATION_PENDING`, `VERIFIED`, `IPS_PIN_SETTING`, `IPS_PIN_SET`, `ALIAS_REGISTRATION_PENDING`, `ALIAS_REGISTERED`, `READY_FOR_IPP_PAYMENTS`, `SUSPENDED`, `DEREGISTERED`

**Legacy status values** (kept for existing records): `step_1_identity`, `step_2_bank_details`, `step_3_documents`, `step_4_vpa_selection`, `step_5_review`, `step_6_submitted`, `step_7_approved`, `rejected`
