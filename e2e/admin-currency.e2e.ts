...
async function assertNoStrayDollar(
  ...
  expect(
    mismatches,
    `${pageName}: found stray $ amounts: ${JSON.stringify(mismatches.slice(0, 5))}`
  ).toHaveLength(0);
...
const tabs: Array<{ id: string; label: string; requireNad?: boolean }> = [
  { id: 'overview', label: 'Financial' },
  { id: 'payments', label: 'Payments' },
  { id: 'approvals', label: 'Approvals', requireNad: false },
...
test.describe('Admin currency formatting (N$ with 2 decimals)', () => {
  test('No stray $ and N$ present on key tabs', async ({ page }) => {
    ...
    if (role !== 'admin')
      test.skip(true, 'Admin credentials not available; skipping admin currency test');
