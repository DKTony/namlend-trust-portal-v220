-- Migration: Restrict process_approval_transaction to staff roles only
-- Description: Removes 'approver' role from process_approval_transaction RPC.
--              The approver role should approve/reject requests, but not execute
--              the final loan creation step (financial operation requiring staff privileges).
--              Maintains all security fixes from 20260108070000 (FOR UPDATE + status validation).
-- Created: 2026-01-08
-- Severity: LOW - Access scope correction
-- References: Original design in 20250925_add_process_approval_transaction_rpc.sql

-- Recreate function with staff-only authorization (admin, loan_officer)
...
DECLARE
  ...
  v_user_role TEXT;
BEGIN
  -- Verify caller is authorized (admin or loan_officer ONLY - staff roles)
  -- Approvers can approve/reject but not execute final loan creation
  ...
  IF v_user_role IS NULL THEN
    RETURN json_build_object(
      'success', false, 
      'error', 'Unauthorized: Only admins and loan officers can process approvals'
    );
  END IF;

  -- CRITICAL: Lock and fetch the approval request with FOR UPDATE
  -- This prevents concurrent modifications to the approval request status
  SELECT ar.*, ar.user_id, ar.request_type, ar.request_data, ar.status
  INTO v_request
  ...
  -- CRITICAL: Validate approval request is in 'approved' status
  -- This prevents loan creation from pending/rejected requests
  IF v_request.status != 'approved' THEN
    RETURN json_build_object(
      'success', false, 
      'error', format('Request must be in approved status to process. Current status: %s', v_request.status)
    );
  END IF;
...
COMMENT ON FUNCTION public.process_approval_transaction IS 
  'Processes an approved loan application and creates the loan record. 
   SECURITY: Staff-only (admin, loan_officer). Validates status = approved before processing.
   CONCURRENCY: Uses FOR UPDATE row locking + 23505 exception handler for idempotency.
   Fixed 2026-01-08 to restore status validation and restrict to staff roles only.';
