-- Migration: 20260107120000_create_payment_rpc
-- Purpose: Create RPC function for client payments with audit trail, ledger event, and fee recording
-- Fixes: CRITICAL - Payment bypass issue where payments were inserted directly without audit/ledger

-- Add processing_fee column to payments table if not exists
ALTER TABLE public.payments 
ADD COLUMN IF NOT EXISTS processing_fee NUMERIC(10,2) DEFAULT 0;

-- Add idempotency_key column for preventing duplicate payments
ALTER TABLE public.payments 
...
-- Create the create_payment RPC function
...
BEGIN
  ...
  -- Create payment record with fee
...
Supports idempotency via idempotency_key parameter. Returns existing payment if idempotency key matches.';
