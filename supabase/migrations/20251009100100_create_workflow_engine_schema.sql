...
-- Partial unique index for one active workflow per entity type
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_active_workflow 
  ON workflow_definitions(entity_type) WHERE is_active = true;
...
-- Partial unique index for one active workflow per entity
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_active_workflow_instance 
...
-- Note: Policy for users to view their workflow instances is created after workflow_stage_executions table
...
ON CONFLICT (entity_type) WHERE is_active = true
DO NOTHING;
