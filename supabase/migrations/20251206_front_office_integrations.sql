...
-- Notification Templates (read by all, write by admin)
...
CREATE POLICY "Admins can manage notification queue" ON notification_queue
    FOR ALL USING (
        EXISTS (SELECT 1 FROM profiles WHERE user_id = auth.uid() AND role IN ('admin', 'loan_officer'))
    );
...
INSERT INTO notification_templates (code, name, category, channels, title, body, priority, variables) VALUES
    ('LOAN_SUBMITTED', 'Loan Application Submitted', 'loan', ARRAY['in_app', 'sms', 'email'], 
     'Application Submitted', 'Your loan application for {amount} has been submitted successfully. Reference: {reference}. We''ll review it within 24 hours.',
     'normal', ARRAY['amount', 'reference']),
    
    ('LOAN_APPROVED', 'Loan Approved', 'loan', ARRAY['in_app', 'sms', 'email', 'whatsapp'],
     'Loan Approved! 🎉', 'Great news! Your loan of {amount} has been approved. Funds will be disbursed within 24 hours. Reference: {reference}',
     'high', ARRAY['amount', 'reference']),
    
    ('LOAN_REJECTED', 'Loan Rejected', 'loan', ARRAY['in_app', 'email'],
     'Application Update', 'We regret to inform you that your loan application could not be approved at this time. Please contact us for more information.',
     'high', ARRAY['reference']),
    
    ('LOAN_DISBURSED', 'Loan Disbursed', 'loan', ARRAY['in_app', 'sms', 'email'],
     'Funds Disbursed', 'Your loan of {amount} has been disbursed to your account. First payment of {monthly_payment} is due on {due_date}.',
     'high', ARRAY['amount', 'monthly_payment', 'due_date']),
    
    ('PAYMENT_REMINDER_7', 'Payment Reminder (7 days)', 'payment', ARRAY['in_app', 'sms'],
     'Payment Reminder', 'Your loan payment of {amount} is due in 7 days ({due_date}). Reference: {reference}',
     'normal', ARRAY['amount', 'due_date', 'reference']),
    
    ('PAYMENT_REMINDER_3', 'Payment Reminder (3 days)', 'payment', ARRAY['in_app', 'sms'],
     'Payment Due Soon', 'Your loan payment of {amount} is due in 3 days ({due_date}). Avoid late fees - pay now!',
     'high', ARRAY['amount', 'due_date', 'reference']),
    
    ('PAYMENT_REMINDER_1', 'Payment Reminder (1 day)', 'payment', ARRAY['in_app', 'sms', 'whatsapp'],
     'Payment Due Tomorrow', 'URGENT: Your loan payment of {amount} is due tomorrow ({due_date}). Pay now to avoid late fees.',
     'urgent', ARRAY['amount', 'due_date', 'reference']),
    
    ('PAYMENT_OVERDUE', 'Payment Overdue', 'collections', ARRAY['in_app', 'sms', 'whatsapp', 'email'],
     'Payment Overdue', 'Your payment of {amount} was due on {due_date}. Please pay immediately to avoid additional charges.',
     'urgent', ARRAY['amount', 'due_date']),
    
    ('PAYMENT_RECEIVED', 'Payment Received', 'payment', ARRAY['in_app', 'sms'],
     'Payment Received ✓', 'Thank you! We received your payment of {amount}. Remaining balance: {balance}. Reference: {reference}',
     'normal', ARRAY['amount', 'balance', 'reference']),
    
    ('LOAN_COMPLETED', 'Loan Completed', 'loan', ARRAY['in_app', 'sms', 'email'],
     'Loan Paid Off! 🎉', 'Congratulations! Your loan has been fully repaid. Thank you for choosing NamLend Trust. Apply again anytime!',
     'normal', ARRAY[]),
    
    ('KYC_APPROVED', 'KYC Approved', 'kyc', ARRAY['in_app', 'email'],
     'Verification Complete', 'Your identity has been successfully verified. You can now apply for loans.',
     'normal', ARRAY[]),
    
    ('KYC_REJECTED', 'KYC Rejected', 'kyc', ARRAY['in_app', 'email'],
     'Verification Issue', 'We could not verify your documents. Please upload clearer copies and try again.',
     'high', ARRAY['reason'])
