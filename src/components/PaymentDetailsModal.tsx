        ...
        <DialogHeader className="p-6 border-b border-border bg-background/95 backdrop-blur-xl sticky top-0 z-10">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
               ...
               <div>
                  <DialogTitle className="text-xl font-bold tracking-tight text-foreground">Payment Receipt</DialogTitle>
                  <p className="text-muted-foreground text-xs mt-0.5 font-mono">ID: {payment.id.slice(0, 8)}</p>
               </div>
            </div>
             ...
             <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                   <div className="flex items-center gap-2 text-muted-foreground mb-1">
                      ...
                      <span className="text-xs font-medium uppercase tracking-wider">Payment Amount</span>
                   </div>
                   <p className="text-4xl font-bold text-foreground tracking-tight">{formatNAD(payment.amount)}</p>
                </div>
                <div className="text-left md:text-right">
                   ...
                   <p className="text-xl font-semibold text-foreground capitalize tracking-wide">{payment.payment_method}</p>
                </div>
             </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
             ...
             <div className="space-y-4">
                ...
                <div className="bg-muted/30 rounded-xl border border-border p-1">
                   {[
                     { label: 'Loan ID', value: payment.loan_id.slice(-8), icon: FileText, mono: true },
                     { label: 'Payment ID', value: payment.id.slice(-8), icon: Hash, mono: true },
                     { label: 'Reference', value: payment.reference_number || 'N/A', icon: Hash, mono: true },
                   ].map((item, i) => (
             ...
             <div className="space-y-4">
                ...
                <div className="space-y-4 pl-2">
                   {[
                     ...
                     { label: 'Completed', date: payment.paid_at, icon: CheckCircle, active: !!payment.paid_at, highlight: true }
                   ].map((step, i) => (
                      <div key={i} className={cn("flex items-center gap-3", !step.active && "opacity-40 grayscale")}>
                         ...
                         <div>
                            <p className={cn("text-xs font-medium", step.highlight ? "text-green-600 dark:text-green-400" : "text-foreground")}>{step.label}</p>
