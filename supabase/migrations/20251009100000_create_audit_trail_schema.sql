...
-- Note: audit_logs table already exists from earlier migration
-- This migration adds missing columns if they don't exist
-- ============================================================================

-- Add missing columns to existing audit_logs table (skip if exists)
DO $$
BEGIN
  -- Add timestamp column if it doesn't exist (maps to created_at)
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'audit_logs' AND column_name = 'old_state') THEN
    ALTER TABLE public.audit_logs ADD COLUMN old_state JSONB;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'audit_logs' AND column_name = 'new_state') THEN
...
-- RLS already enabled from earlier migration, skip
-- Policy already exists from earlier migration, skip
...
CREATE TABLE IF NOT EXISTS state_transitions (
  ...
  workflow_instance_id UUID, -- FK to workflow_instances added later if needed
  metadata JSONB DEFAULT '{}'::jsonb
);
