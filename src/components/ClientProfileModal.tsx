...
export const ClientProfileModal: React.FC<ClientProfileModalProps> = ({
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <Badge variant="outline" className={cn("flex items-center space-x-1.5 px-2.5 py-0.5 font-medium", variant.className)}>
        {variant.icon}
                  ...
                  <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                    <div className="flex items-start gap-5">
                      ...
                      <div>
                        <div className="flex items-center gap-3">
                          ...
                          <Badge variant={profile.verified ? "default" : "outline"} className={cn(
                    ...
                    <div className="flex gap-4">
                      <div className="bg-background/50 rounded-xl p-3 px-4 border border-border">
                         <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold mb-1">Monthly Income</p>
                         <p className="text-lg font-bold text-foreground">{formatNAD(profile.monthly_income || 0)}</p>
                      </div>
                      <div className="bg-background/50 rounded-xl p-3 px-4 border border-border">
                         <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold mb-1">Credit Score</p>
                         <p className="text-lg font-bold text-blue-500 dark:text-blue-400">{profile.credit_score || 'N/A'}</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <Tabs defaultValue="loans" className="w-full">
                ...
                <TabsContent value="loans" className="m-0 space-y-3">
                  {loans.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {loans.map((loan) => (
                        <div key={loan.id} className="group flex items-center justify-between p-4 bg-card hover:bg-accent/50 rounded-xl border border-border hover:border-accent transition-all cursor-pointer">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <div className="flex items-center gap-3">
                                <span className="font-bold text-foreground">{formatNAD(loan.amount)}</span>
                                {getStatusBadge(loan.status)}
                              </div>
                              <div className="flex items-center gap-3 text-xs text-muted-foreground mt-1">
                                <span className="flex items-center gap-1"><Calendar className="h-3 w-3" /> {loan.term_months}mo</span>
                                <span className="flex items-center gap-1"><TrendingUp className="h-3 w-3" /> {loan.interest_rate}%</span>
                                <span className="flex items-center gap-1"><FileText className="h-3 w-3" /> {loan.purpose}</span>
                              </div>
                            </div>
                          </div>
                          <div className="flex items-center gap-4">
                            <div className="text-right">
                              <p className="text-xs text-muted-foreground mb-0.5">Applied {formatDate(loan.created_at)}</p>
                              <p className="text-[10px] font-mono text-muted-foreground">ID: {loan.id.slice(0, 8)}</p>
                            </div>
                  ...
                  {payments.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {payments.map((payment) => (
                        <div key={payment.id} className="group flex items-center justify-between p-4 bg-card hover:bg-accent/50 rounded-xl border border-border hover:border-accent transition-all">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <div className="flex items-center gap-3">
                                <span className="font-bold text-foreground">{formatNAD(payment.amount)}</span>
                                {getStatusBadge(payment.status)}
                              </div>
                              <div className="flex items-center gap-3 text-xs text-muted-foreground mt-1">
                                <span className="flex items-center gap-1 capitalize"><CreditCard className="h-3 w-3" /> {payment.payment_method}</span>
                                {payment.reference_number && (
                                  <span className="flex items-center gap-1 font-mono"><FileText className="h-3 w-3" /> {payment.reference_number}</span>
                                )}
                              </div>
                            </div>
                          </div>
                          <div className="text-right">
                            <p className="text-xs text-muted-foreground mb-0.5">{formatDate(payment.created_at)}</p>
                            <p className="text-[10px] font-mono text-muted-foreground">ID: {payment.id.slice(0, 8)}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <TabsContent value="documents" className="m-0">
                  <div className="p-12 text-center text-muted-foreground border border-dashed border-border rounded-xl">
                    ...
                    <p className="text-xs text-muted-foreground mt-2">KYC documents and contracts will appear here.</p>
                  </div>
                </TabsContent>
                   ...
                   {activities.length === 0 ? (
                    ...
                    <div className="space-y-2">
                       {activities.map((activity) => (
                        <div key={activity.id} className="flex items-center justify-between p-4 bg-card rounded-xl border border-border">
                           <div className="flex items-center gap-4">
                              ...
                              <div>
                                <div className="flex items-center gap-2">
                                  <span className="font-medium text-foreground">{activity.request_type}</span>
                                  {getStatusBadge(activity.status)}
                                </div>
                                <p className="text-xs text-muted-foreground mt-0.5">Priority: <span className="capitalize text-foreground">{activity.priority}</span></p>
                              </div>
                           </div>
                           <span className="text-xs text-muted-foreground font-mono">{formatDate(activity.created_at)}</span>
                        </div>
                       ))}
                    </div>
                  )}
