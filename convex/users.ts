...
export const assignRole = mutation({
  args: {
    ...
    // Legacy `admin` rows remain readable, but new tenant assignments use `tenant_admin`.
    role: v.union(v.literal('client'), v.literal('loan_officer'), v.literal('tenant_admin')),
  },
  returns: v.null(),
  handler: async (ctx, { targetUserId, role }) => {
    ...
    if (
      ...
      existing.institutionId !== profile.institutionId
    ) {
      throw new ConvexError({
        code: 'FORBIDDEN',
        message: 'The target user has conflicting tenant bindings.',
      });
    }

    const targetInstitutionId = existing?.institutionId ?? profile?.institutionId;
    await assertOwnerOrTenantStaff(ctx, targetUserId, targetInstitutionId);

    const institutionId = targetInstitutionId ?? callerTenant.institutionId ?? undefined;
