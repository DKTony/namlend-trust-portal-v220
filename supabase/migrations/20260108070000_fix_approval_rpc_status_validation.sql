-- Migration: Fix approval RPC status validation regression
-- Description: Restores critical status validation and row locking that was accidentally removed
--              in 20260108060000. Maintains 23505 idempotency handler while re-adding:
--              1. FOR UPDATE row locking to prevent concurrent modification
--              2. status = 'approved' validation to prevent loan creation from pending/rejected requests
-- Created: 2026-01-08
-- Severity: HIGH - Security regression fix

-- Drop and recreate the function with proper status validation + 23505 handling
...
DECLARE
  ...
  v_user_role TEXT;
BEGIN
  -- Verify caller is authorized (admin, loan_officer, or approver)
  ...
  -- CRITICAL FIX: Lock and fetch the approval request with FOR UPDATE
  ...
  -- CRITICAL FIX: Validate approval request is in 'approved' status
...
COMMENT ON FUNCTION public.process_approval_transaction IS 
  'Processes an approved loan application and creates the loan record. 
   SECURITY: Validates status = approved before processing.
   CONCURRENCY: Uses FOR UPDATE row locking + 23505 exception handler for idempotency.
   Fixed 2026-01-08 to restore status validation regression.';
