...
- Maximum loan disbursement that can be sent in a _single IPN transaction_.
...
- Internal workflows (e.g. marking loans as _settled_) must respect **finality semantics** of IPN – only treat a payment as final when IPP indicates **SUCCESS** (and any scheme-specific reconciliation conditions are met).
